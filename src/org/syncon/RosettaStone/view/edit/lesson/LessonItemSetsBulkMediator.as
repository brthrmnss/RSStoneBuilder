package org.syncon.RosettaStone.view.edit.lesson
{
	...
	import org.syncon.RosettaStone.view.edit.LessonItemSets;
	...
	public class LessonItemSetsBulkMediator extends Mediator 
	{
		[Inject] public var ui : LessonItemSetsBulk ;
		...
		override public function onRegister():void
		{
			...
			*/
			...
			this.ui.addEventListener( LessonItemSetsBulk.ALL, this.onAll ) 				
			this.ui.addEventListener( LessonItemSetsBulk.NONE, this.onNone ) 
			this.ui.addEventListener( LessonItemSetsBulk.RANGE, this.onRange ) 		
			
			this.ui.addEventListener( LessonItemSetsBulk.BULK_UPDATE, this.onBulkUpdate ) 		
				
			this.ui.addEventListener( LessonItemSetsBulk.BULK_UPDATE_MANUAL, this.onBulkUpdateManual  ) 		
				
			//this.ui.addEventListener(   LessonItemSets.ADD_NEW_LESSON_SET, this.onAddNewLessonSet ) 	 
		}
		
		protected function onBulkUpdateManual(event:Event):void
		{
			this.dispatch( new ShowPopupEvent(ShowPopupEvent.SHOW_POPUP, 
				'PopupSearchWord', [this.model.currentLessonItem.itemRendererEdit,
					this.currentLesson.selectedItems ],'goTo') ) ; 
		}
		
		protected function onRange(event:Event):void
		{
			var selectedMode:Boolean;
			for each ( var i : LessonItemVO in this.currentLesson.items ) 
			{
		...
		protected function onInvertSelection(event:Event):void
		{
			for each ( var i : LessonItemVO in this.currentLesson.items ) 
			{
				i.editSelected = ! i.editSelected; //false
				i.update()
			}
		}
		protected function onNone(event:Event):void
		{
			for each ( var i : LessonItemVO in this.currentLesson.items ) 
			{
				
				i.editSelected = false
				i.update()
			}
		}
		
		
		protected function onBulkUpdate(event:Event):void
		{
				//this.dispatch( new HidePopupEvent(HidePopupEvent.HIDE_POPUP, 'PopupSearchWord' ) )
				this.dispatch( new ShowPopupEvent(ShowPopupEvent.SHOW_POPUP, 
					'PopupUpdateItemsBulk', [null, '', this.currentLesson.selectedItems ],'goTo') ) ; 
		}
		
		
		protected function onAll(event:Event):void
		{
			for each ( var i : LessonItemVO in this.currentLesson.items ) 
			{
		...
		/**
		 * when quickinput importing, we input into current lesson, 
		 * make a new lesson set first if one is not selected .....
		 * */
		/*protected function onAddNewLessonSet(event:Event):void
		{
