package  org.syncon.RosettaStone.view.ellips
{
	...
	/**
	 * Use the other mediator
	 * */
	public class PlayerAutomateMediator extends Mediator 
	{
		[Inject] public var ui :  IAutomateMediator;
		...
		override public function onRegister():void
		{
			...
			*/
			...
			this.ui.addEventListener( AutomateUIEvent.RESTART, this.onRestart ) 
			this.ui.addEventListener( AutomateUIEvent.NEXT, this.onNextItem ) 				
			
			this.ui.addEventListener( AutomateUIEvent.NEXT_SET, this.onSkipToNextSet ) 		
			this.ui.addEventListener( AutomateUIEvent.DONE, this.onTestDone ) 		
			this.timerDelayIntroduction.addEventListener(TimerEvent.TIMER, this.onDelayIntroductionComplete , false, 0, true)
			
			this.ui.addEventListener( AutomateUIEvent.ACTIVE_CHANGED, this.onActiveChanged ) 		
		...
		protected function onActiveChanged(event:Event):void
		{
			if ( this.ui['active'] == false ) 
			{
				this.stopEverything();
				//...this is where that pause came in ..
			}
			else
			{
		...
		public function onUpdateSettings(e:AutomateEvent):void{
			this.option_introduceItems = e.lesson.option_introduceItems; 
			this.option_oneShotAnswerAttemps
			
			var options : Array = [	'option_showPromptOnItemRender'  ,
				'option_showPromptText',
				'option_introduceItems',
				'option_oneShotAnswerAttemps',  //why would you want this ? or maybe counting how many times
				'option_PlaySoundWhenWrongOneClicked' ,
				'option_MasteryMode']
		...
		private function onNextLesson(e: AutomateEvent): void
		{
			...
			*/
			...
			if ( nextLesson.retrievedContentsOnce == false ) 
			{
				this.dispatch( new  LoadLessonTriggerEvent(LoadLessonTriggerEvent.LOAD_SOUNDS,
					nextLesson ,'', this.onCurrentLessonLoaded ));//, lessonBaseDir )) ; 
				return
			}
		...
		import org.syncon.RosettaStone.vo.LessonVO;
		public function onCurrentLessonLoaded( b : LessonVO ) : void
		{
			this.model.currentLesson = b
			this.currentLesson = this.model.currentLesson ;
			//if ( this.ui.autoStart == false ) 
			this.onRestart(null) 
		}
		...
		private var _modeIntroducing:Boolean;
		
		public function get modeIntroducing():Boolean
		{
			return _modeIntroducing;
		}
		
		public function set modeIntroducing(value:Boolean):void
		{
			_modeIntroducing = value;
		}
		...
		private function introduceItems(fxDone:Function, wait  : Boolean = true ):void
		{
			...
			if ( this.option_introduceItems ) 
			{
				...
				gIntroduce.go( this.currentLessonSet.items.toArray(), introduceItem, this.onDoneIntroducingItems,1000 )
			}
			else
			{
		...
		*/
		public function introduceItem( item : LessonItemVO/*, wait : Boolean = true */) : void
		{
			...
			*/
			...
			this.model.playLessonItem( item, this.gIntroduce.next) ; 
		}
		...
		private function updateDisplay():void
		{
			...
			*/
			...
			//	this.ui.dispatchEvent( new  AutomateUIEvent(AutomateUIEvent.UPATE_DISPLAY, ) ) 
			/*
		...
		{
			this.ui['txtCurrentSet'].text = '';//this.currentLesson.name;
		...
		{
			//clear the fx call back function
