package  org.syncon.RosettaStone.view.mobile.alarm
{
	...
	public class  EditVoiceViewMediator extends Mediator
	{
		[Inject] public var ui:  EditVoiceView;
		[Inject] public var model :  RSModel;
		private var styler:StyleConfigurator=new  StyleConfigurator();
		private var ev :  LazyEventHandler = new LazyEventHandler(); 
		private var alarm: VoiceVO;
		public function EditVoiceViewMediator()
		{
		...
		override public function onRegister():void
		{
			...
			ev.init( this.ui ) ; 
			ev.addEv( EditVoiceView.EDIT_NAME, this.onEditName ) ; 
			ev.addEv( EditVoiceView.EDIT_REPEAT, this.onEditRepeat ) ; 
			
			ev.addEv( EditVoiceView.DONE, this.onDone ) ; 
			ev.addEv( EditVoiceView.DELETE, this.onDelete ) ; 
			ev.addEv( BackBtn.GO_BACK_BTN, this.onDone ) ; 				
			
			ev.addEv( EditVoiceView.EDIT_SOUND, this.onEditSound ) ; 	
			ev.addEv( EditVoiceView.CHANGED_ENABLED, this.onChangeEnabled ) ; 
			ev.addEv( EditVoiceView.EDIT_TIME, this.onEditTime ) ; 				
			ev.addEv( FlexEvent.DATA_CHANGE, this.onDataChange )
				
			ev.addEv( SoundRenderer.PLAY_SOUND, this.onPlaySound ) ; 				
			ev.addEv(SoundRenderer.REC_SOUND, this.onRecordSound )
			...
			this.alarm = this.ui.data as VoiceVO; 
			this.onLoadVoice();
			this.onUpdateForm(); 
		}
		
		private function onRecordSound(e:CustomEvent):void
		{
			// TODO Auto Generated method stub
			
			this.dispatch(   NavigateCommandTriggerEvent.pushView( EditSampleView , e.data) ) ; 
		}
		
		private function onPlaySound(e:CustomEvent):void
		{
			var event_ : ShowPopupEvent = new ShowPopupEvent(ShowPopupEvent.SHOW_POPUP, 
				'PopupPlaySample', [e.data], 'open3' )   		
		...
		/*	if ( e.length == 0 || this.alarm.repeat == '' || this.alarm.repeat == null  ) 
			...
			*/
			...
			this.ui.txtDesc.text = this.alarm.name; 
			//images ...
			this.ui.image.source = this.alarm.pic 
		/*	
